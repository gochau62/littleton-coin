...
<script type="text/javascript">
	document.title = "Import Order File";
	...
	function myFunction() {
		...
		if ($('#myFile').get(0).files.length === 0){ 
			swal("No file attached", "You must attach an XLSX file containing in order: username, sequence #, comment field, and active/inactive - attach a file and try again", "error");
			return;
		}
		...
		if (bool == false) { 
			...
			// sends ajax request to url (AD_importOrderFile_ajax.php)
			// response is process in the success function
			$.ajax({
				url: 'AD_importOrderFile_ajax_GORDON.php',
				data: formData,
			    contentType: false,
			    datatype: 'text',
			    processData: false,
				...
				success: function(rtnData) {
					...
					if (parsed['returnClass'] == "error") {
						...
						swal("Some errors occurred", "Some records resulted in errors - see your e-mail for more info\n\n Records successfully inserted: " + parsed['insertCount'] + "\n Records not inserted: " + parsed['errorCount'], "warning");
						
					} else if (parsed['returnClass'] == "success") {
						swal("All records uploaded", "All records have been uploaded/updated successfully \n\n Records inserted: " + parsed['insertCount'], "success");
...
$authorized = chkAutUsr($authConn, $user, "LCCONLINE", 50);
 ...
 if ( $authorized != "yes") {
	...
	include("AD_importOrderFile_dsp_GORDON.php"); 
	$screenData = "";
