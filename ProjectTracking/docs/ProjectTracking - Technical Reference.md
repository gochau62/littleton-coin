...
Project-number links are plain relative: `PROJ_ctl.php` sits in the same
folder as these files, so `projUrl(num)` writes nothing but the name and
the number. An earlier version sent dev links out to production over
`prjLegacyBase()`; that was dropped once everything runs in one place.
The pages still call `projUrl(num)` rather than writing the path
themselves, so a change of location is one line.
